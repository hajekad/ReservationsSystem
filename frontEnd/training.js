async function trainingGet() {
    ...
    try {
      const response = await fetch(`http://localhost:6060/training?id=${id}`);
      if (response.ok) {
        const training = await response.json();
        console.log("Training:\n" + JSON.stringify(training, null, 2));
        document.getElementById('responseGet').value = JSON.stringify(training, null, 2);
      } else {
...
async function createTraining() {
    ...
    try {
      const response = await fetch('http://localhost:6060/training', {
        method: 'POST',
      ...
      if (response.ok) {
        const training = await response.json();
        console.log("Training:\n" + JSON.stringify(training, null, 2));
        document.getElementById('responsePost').value = JSON.stringify(training, null, 2);
      } else {
        console.error(`Error: ${response.status} ${response.statusText}`);
...
async function updateTraining() {
    ...
    console.log("Sending: " + JSON.stringify(trainingDto));
